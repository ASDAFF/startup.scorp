...
/**
 * SCorp module
 * @copyright 2015 - 2017 Aspro
 */

if(!defined('SCORP_MODULE_ID')){
	define('SCORP_MODULE_ID', 'aspro.scorp');
}
			...
			if($socket){
				...
				$arData = json_encode(
					array(
						...
						'bitrix_edition' => $GLOBALS['APPLICATION']->ConvertCharset(self::getBitrixEdition(), SITE_CHARSET, 'utf-8'),
						'bitrix_key_hash' => md5(CUpdateClient::GetLicenseKey()),
						'site_name' => $GLOBALS['APPLICATION']->ConvertCharset(COption::GetOptionString('main', 'site_name'), SITE_CHARSET, 'utf-8'),
						'site_url' => $GLOBALS['APPLICATION']->ConvertCharset(COption::GetOptionString('main', 'server_name'), SITE_CHARSET, 'utf-8'),
						'email_default' => $GLOBALS['APPLICATION']->ConvertCharset(COption::GetOptionString('main', 'email_from'), SITE_CHARSET, 'utf-8'),
						'action' => $action,
					)
				);
