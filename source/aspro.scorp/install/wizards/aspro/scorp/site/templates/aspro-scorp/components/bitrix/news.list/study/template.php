			...
			<?foreach($arResult['ITEMS'] as $i => $arItem):?>
				...
				<?ob_start();?>
					...
					<?ob_start();?>
						...
						<?endif;?>
					<?$pricePart = ob_get_clean();?>

					<?// element display properties?>
					<?$arDisplayPropertiesCodes = array_diff(array_keys($arItem['DISPLAY_PROPERTIES']), array('PRICE', 'PRICEOLD', 'PERIOD', 'PHOTOS', 'DOCUMENTS', 'LINK_GOODS', 'LINK_STAFF', 'LINK_REVIEWS', 'LINK_PROJECTS', 'LINK_SERVICES', 'FORM_ORDER', 'FORM_QUESTION', 'PHOTOPOS'));?>
					<?if($arItem['DISPLAY_PROPERTIES'] && $arDisplayPropertiesCodes):?>
						<hr />
				...
				<?ob_start();?>
					<?if($bImage):?>
						<div class="image">
							...
							<?elseif($arItem['FIELDS']['DETAIL_PICTURE']):?><a href="<?=$imageDetailSrc?>" alt="<?=($bImage ? $arItem['PREVIEW_PICTURE']['ALT'] : $arItem['NAME'])?>" title="<?=($bImage ? $arItem['PREVIEW_PICTURE']['TITLE'] : $arItem['NAME'])?>" class="img-inside blink fancybox">
							<?endif;?>
