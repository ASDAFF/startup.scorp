							...
							<?if(strlen($arItem['DISPLAY_PROPERTIES']['PRICE']['VALUE']) || $bOrderButton || $bBuyButton):?>
								<div class="<?=($bShowImage ? 'col-md-3 col-sm-4 col-xs-12' : 'col-md-2 col-sm-3 col-xs-12')?>">
									<div class="foot">
										...
										<?if($bOrderButton):?>
											<span class="btn btn-default btn-sm pull-left" data-event="jqm" data-param-id="<?=CCache::$arIBlocks[SITE_ID]['startup_scorp_form']['startup_scorp_order_product'][0]?>" data-product="<?=$arItem['NAME']?>" data-name="order_product"><?=(strlen($arParams['S_ORDER_PRODUCT']) ? $arParams['S_ORDER_PRODUCT'] : GetMessage('S_ORDER_PRODUCT'))?></span>
										<?// element buy block?>
										<?elseif($bBuyButton):?>
