...
if( CModule::IncludeModule("iblock") ){
	...
	if( $bCache && $obFormCache->InitCache( $arParams["CACHE_TIME"], $CACHE_ID, $CACHE_PATH ) ){
		...
		$arResult["EVENT_TYPE"] = "STARTUP_SEND_FORM";
		...
		if( strlen( $_REQUEST["form_submit"] ) > 0 ){
			...
			if( count( $arResult["FORM_ERRORS"] ) <= 0 ){
				//if( check_bitrix_sessid() ){
					...
					foreach( GetModuleEvents("startup.form", "OnBeforeFormSend", true) as $arEvent )
						ExecuteModuleEventEx($arEvent, array(&$arFields));

					if( $RESULT_ID = $el->Add( $arFields ) ){
						$arResult["FORM_RESULT"] = "ADDOK";
						foreach( GetModuleEvents("startup.form", "OnAfterFormSend", true) as $arEvent )
							ExecuteModuleEventEx($arEvent, array(&$arFields));
						...
						if(strlen($arEventFields["EMAIL"])){
							if(isset($arEventFields['ORDER_LIST']) && strpos('startup_scorp_order_page', $arResult['IBLOCK_CODE']) !== false){
